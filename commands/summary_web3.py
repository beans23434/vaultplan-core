...
def summary_web3():
    ...
    summary_table.add_row("ETH In (income)",  str(income["count"]),  f"${income['sum_in']:.2f}")
    summary_table.add_row("ETH Out (expense)", str(expense["count"]), f"${expense['sum_out']:.2f}")
    ...
    for date, ttype, sym, amt, val in recent:
        preview_table.add_row(date, ttype, sym, f"{amt:.8f}", f"${val:.2f}")

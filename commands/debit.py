...
def add_debit(
    ...
    with get_db() as conn:
        ...
        c.execute(
            "INSERT INTO debits (label, amount_due, due_date, account) VALUES (?, ?, ?, ?)",
            (label, amount_due, due_date, account)
        )
        conn.commit()
...
def list_debits(
    ...
    with get_db() as conn:
        ...
        for id_, label, due, paid, date, acct, status in rows:
            table.add_row(str(id_), label, f"${due:.2f}", f"${paid:.2f}", date or "—", acct or "—", status)

    ...
    with get_db() as conn:
        ...
        try:
            ...
            for name, typ, bal in c.fetchall():
                acct_table.add_row(name, typ, f"${bal:,.2f}")
        ...
        try:
            ...
            summary_table.add_row("Income", f"${total_income:,.2f}")
            summary_table.add_row("Expenses", f"${total_expenses:,.2f}")
            summary_table.add_row("Net", f"${net:,.2f}")
        ...
        try:
            ...
            if rows:
                ...
                for tx_type, count, total in rows:
                    tx_table.add_row(tx_type, str(count), f"${total:,.2f}")
        ...
        try:
            ...
            if rows:
                ...
                for date, amount, typ, desc, acc in rows:
                    ...
                    activity_table.add_row(date, typ, f"{sign}${abs(amount):,.2f}", desc, acc)
        ...
        try:
            ...
            if rows:
                ...
                for date, amt, from_acct, desc in rows:
                    ...
                    transfer_table.add_row(date, from_acct, to_acct, f"${amt:,.2f}")
